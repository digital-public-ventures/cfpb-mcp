...
5. **Citation Types**:
   ...
   - `complaint_detail` - Individual complaint (when complaint_id is known)
...
### Phase 5: Local Dataset + Vector Embeddings (MiniLM on Apple Silicon)
...
### Phase 6: Finalize MCP Framework Migration (FastMCP)

*Goal: Standardize the MCP implementation on `FastMCP` after the data/semantic layer is proven.*

1. **Migration timing:** Defer framework-level migration until the end so earlier phases can focus on contracts and data correctness.
2. **Migration scope:** Move MCP tool definitions and wiring to `FastMCP` (and keep REST routes intact), ensuring tool schemas and public HTTP paths remain stable.
3. **Regression guard:** Treat `tests/e2e/**` and the public endpoints (`/openapi.json`, `/complaint/{complaint_id}`, `/mcp/sse`) as compatibility contracts.
