...
import { ToolName, toolDefinitions } from "./definitions.js";
...
export const handlers: Record<ToolName, (args: HandlerArgs) => HandlerResult> = {
  search_complaints: async (args) => {
    ...
    const totalHits = (data as any)?.hits?.total;
  ...
  generate_cfpb_dashboard_url: async (args) => ({ url: buildCfpbUiUrl(args) }),
  ...
  get_overall_trend_signals: async (args) => {
    ...
    return {
      ...
      signals: {
        overall: computeSimpleSignals(points, args.baseline_window, args.min_baseline_mean),
      },
    };
  },
  ...
  rank_group_spikes: async (args) => {
    ...
    scored.sort((a, b) => {
      const aZ = (a as any).signals?.last_vs_baseline?.z;
      const bZ = (b as any).signals?.last_vs_baseline?.z;
  ...
  rank_company_spikes: async (args) => {
    ...
    const topCompanies = companyBucketsFromSearch(searchPayload).slice(0, args.top_n);
    ...
    results.sort((a, b) => {
      const aZ = (a as any).computed?.signals?.last_vs_baseline?.z;
      const bZ = (b as any).computed?.signals?.last_vs_baseline?.z;
      if (aZ === undefined || aZ === null) {
        return 1;
      }
      if (bZ === undefined || bZ === null) {
