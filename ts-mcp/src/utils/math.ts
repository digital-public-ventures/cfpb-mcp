...
export const extractOverallPoints = (payload: unknown): TrendPoint[] => {
  const buckets =
    (payload as Record<string, any>)?.aggregations?.dateRangeArea?.dateRangeArea
      ?.buckets;
  ...
  for (const bucket of buckets) {
    ...
    const key = (bucket as any).key;
    const label = (bucket as any).key_as_string;
    const count = (bucket as any).doc_count;
    if (
...
const extractPointsWithKey = (
  ...
  for (const bucket of trendBuckets) {
    ...
    const keyNum =
      typeof key === "number" ? key : typeof key === "bigint" ? Number(key) : null;
...
export const extractGroupSeries = (
  ...
  const groupBuckets = (payload as any)?.aggregations?.[group]?.[group]?.buckets;
  if (!Array.isArray(groupBuckets)) {
    return [];
  }

  const series: Array<{ group: string; doc_count: number; points: TrendPoint[] }> = [];
  for (const bucket of groupBuckets) {
    if (!bucket || typeof bucket !== "object") {
      continue;
    }
    const groupKey = (bucket as any).key;
    const docCount = (bucket as any).doc_count;
    const trendBuckets = (bucket as any).trend_period?.buckets;
    ...
    const points = pointsWithKey.map(([, label, count]) => [label, count] as TrendPoint);
    series.push({
      group: String(groupKey),
      doc_count: typeof docCount === "number" ? docCount : Number(docCount ?? 0),
      points,
    });
  }
...
export const companyBucketsFromSearch = (
  ...
  const buckets = (payload as any)?.aggregations?.company?.company?.buckets;
  if (!Array.isArray(buckets)) {
    return [];
  }
